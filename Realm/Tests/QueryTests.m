...
- (void)testLinkViewQueryLiveUpdate {
    ...
    RLMArray *employees = [co.employees objectsWhere:@"age = 40"];
    XCTAssertEqualObjects(@"Jill", [[employees lastObject] name]);
    XCTAssertEqual(1U, employees.count);

    [realm beginWriteTransaction];
    [co.employees addObject:eo];
    [realm commitWriteTransaction];

    XCTAssertEqualObjects(@"Joe", [[employees lastObject] name]);
    XCTAssertEqual(2U, employees.count);
}
